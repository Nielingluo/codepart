...
gulp.task('auto', function(){
	...
	//-------------------------编译less-------------------------
	gulp.watch('../less/less/*.less',function (event) {
		var pathsless = watchPath(event,'less','css');
		gulp.src(pathsless.srcPath)
...
// 测试js SourceMap
gulp.task('soujs', function(){
	// 'script/*.js'是即将压缩的js文件
	gulp.src('script/ls.js')
		.pipe(sourcemaps.init())
		.pipe(uglify())
		//给压缩后的文件，添加min后缀名
		.pipe(rename({suffix: '.min'}))
		.pipe(sourcemaps.write('./maps'))
		.pipe(gulp.dest('newScript/js'));
});
